    ...
    <div class="sub_header bg01">
      <div class="wrap">
        <div class="location">
          ...
          <span>&gt;</span>
          <p title="현재페이지">{{ $t('nav.dropNav.nav6.n0') }}</p>
        </div>
        <h2 class="tit01 color_mid_blue">{{ $t('nav.dropNav.nav6.n0') }}</h2>
      ...
      <div class="con_body">
        <div class="total_infor">
          <p class="block txt03">
            ...
            <span
              ...
              v-text="$store.state.notice.data.currentlist.pageinfo.totalcount"
            ></span>
            {{ $t('board.count1') }}
          </p>
        ...
        <template v-if="$store.state.notice.data.currentlist.pageinfo.totalcount != 0">
          <table class="tbl02 col">
            ...
            <tbody>
              <tr
                v-for="(obj, index) in this.$store.state.notice.data.currentlist.noticelist"
                :key="index"
              >
                <td>{{ itemNo - index }}</td>
                <td>
        ...
        <div class="paging">
          <a
            v-show="$store.state.notice.data.currentlist.pageinfo.currentpage > 10"
            class="btn_pg_first"
            @click="paging($store.state.notice.data.currentlist.pageinfo.firstpage)"
            >처음페이지</a
          >
          <a
            v-show="$store.state.notice.data.currentlist.pageinfo.currentpage > 10"
            class="btn_pg_prev"
            @click="paging($store.state.notice.data.currentlist.pageinfo.prevpage)"
            >이전페이지</a
          >
          <a
            v-bind:class="obj == $store.state.Page ? 'active' : ''"
            v-for="(obj, index) in this.$store.state.notice.data.currentlist.pageinfo.disppage"
            :key="index"
            @click="paging(obj)"
          >
            {{ obj }}
          ...
          <a
            v-show="
              parseInt($store.state.notice.data.currentlist.pageinfo.lastpage / 10) !=
                parseInt(($store.state.notice.data.currentlist.pageinfo.currentpage - 1) / 10)
            "
            class="btn_pg_next"
            @click="paging($store.state.notice.data.currentlist.pageinfo.nextpage)"
            >다음페이지</a
          >
          <a
            v-show="
            ...
            "
            class="btn_pg_last"
            @click="paging($store.state.notice.data.currentlist.pageinfo.lastpage)"
            >마지막페이지</a
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    info(id) {
      ...
      this.$store.dispatch('INFO_NOTICEINFO', { id: this.$store.state.Id });
    },
    search() {
      if (this.searchtext == '') {
  ...
  created() {
    ...
    this.$store
      ...
      .then(() => {
        ...
        this.item = this.$store.state.notice;
      });
  },
};
</script>
